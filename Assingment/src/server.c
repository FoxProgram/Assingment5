	...
	while (1) {
		...
		if ((pid = fork()) == 0) {
			...
			if (!file_exists(filepath)) {
				...
				memset(&buf, 0, sizeof(buf));
				sprintf(buf,
						"<html><head><title>404 Not Found</title></head><body bgcolor=\"white\"><center><h1>404 Not Found</h1></center><hr><center>Powered by %s</center></body></html>",
						"fasthttp");
				write(client_sock, buf, strlen(buf));
				close(client_sock);
				memset(&buf, 0, sizeof(buf));
			...
			if (is_dir(filepath)) {
				memset(&tmp, 0, sizeof(tmp));
				sprintf(tmp,
						"<html><head><title>Index of %s</title></head><body><h1>Index of %s</h1><ul><li><a href=\"/\"> Parent Directory</a></li>",
						filepath, filepath);
				strcat(buf, tmp);
...
void parser(char *s, char res[][255], char host[][255]) {
	...
	char hosts[255];
	for (i = 0; s[i] != '\r'; i++)
		/* obtain the first line in http protocol head */
		;
	s[i] = '\0';
	n = i++;
...
static int WriteLog(const char *message) {
	...
	if (g_log_fd == 0) {
		...
		if ((g_log_fd = open(g_log_path, O_RDWR | O_CREAT | O_APPEND,
